...
export function AboutSection({ onOpenInquiry, isArabic = true }: AboutSectionProps) {
  ...
  return (
    <section
      id="why-us"
      className="relative py-24 px-4 bg-[#FAFAFA] overflow-hidden font-ibm border-t border-neutral-200"
      ref={heroRef}
    >
      {/* SVG Clip Paths */}
      ...
      />
      <TimelineContent
        className="absolute bottom-0 left-0 right-0 top-0 bg-[linear-gradient(to_right,#a3a3a31a_1px,transparent_1px),linear-gradient(to_bottom,#a3a3a31a_1px,transparent_1px)] bg-[size:70px_70px] [mask-image:radial-gradient(ellipse_80%_50%_at_50%_100%,#000_70%,transparent_110%)]"
        animationNum={3}
        customVariants={revealVariants3}
        timelineRef={heroRef}
      />
      ...
      {/* Main Container */}
      <div className="relative z-10 max-w-4xl mx-auto text-center font-ibm">
        {/* Section Badge Tag */}
        <div className="text-[#0D5C3A] text-xs font-bold uppercase tracking-widest mb-4 flex items-center justify-center gap-2">
          {isArabic ? "عن منصة أمـلاك" : "ABOUT AMLAK PLATFORM"}
        </div>

        {/* Vertical Cut Reveal Animated Headline with Emerald Underline on 1 Word */}
        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-neutral-900 mb-6 leading-[1.25] tracking-tight">
          <VerticalCutReveal
            splitBy="words"
            ...
            containerClassName="text-neutral-900 leading-[1.25] text-center justify-center items-center font-ibm"
          >
            {isArabic
              ? "معاينة وسهولة في الشراء مباشرة لكل عقار وسيارة"
              : "Easy viewing and direct purchase for properties and cars"}
          </VerticalCutReveal>
        </h2>

        {/* Friendly Copywriting Paragraph */}
        <TimelineContent
          as="p"
          animationNum={0}
          customVariants={revealVariants}
          timelineRef={heroRef}
          className="text-neutral-600 text-center sm:text-lg text-base mb-8 max-w-2xl mx-auto leading-relaxed font-normal"
        >
          {isArabic
      ...
      {/* 4 Clip-Path Animated Showcase Images (Luxury Assets) */}
      <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 pt-16 lg:h-[26rem] md:h-[22rem] sm:h-[18rem] h-[16rem]">
        {/* Top Left Image */}
        <TimelineContent
          as="figure"
          animationNum={2}
          timelineRef={heroRef}
          customVariants={revealVariants}
          className="w-full h-full rounded-2xl overflow-hidden shadow-lg border border-neutral-200"
          style={{ clipPath: "url(#clip-squiggle)" }}
        >
          <img
            src="/images/about_home.jpeg"
            alt="عقارات أملاك الفاخرة"
            width={300}
            height={400}
            className="object-cover w-full h-full rotate-3 hover:rotate-0 transition-transform duration-500"
          />
        </TimelineContent>
        ...
        >
          <img
            src="/images/jabal_amman.jpg"
            alt="شقق جبل عمان الراقية"
            width={300}
            height={400}
            className="object-cover w-full h-full rotate-3 hover:rotate-0 transition-transform duration-500"
          />
