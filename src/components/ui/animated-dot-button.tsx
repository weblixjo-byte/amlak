...
export function AnimatedDotButton({
  ...
  return (
    <motion.button
      ...
      whileHover="hover"
      className={cn(
        "group relative flex w-fit items-center gap-3 overflow-hidden rounded-full px-7 py-3.5 font-bold text-white shadow-lg transition-colors cursor-pointer border-0 outline-none ring-0 focus:outline-none focus:ring-0 active:outline-none",
        buttonBg,
        className
      )}
      {...props}
    >
      {/* Expanding Circle Dot positioned at the inner corner padding (Z-0) */}
      <motion.div
        className={cn(
          "absolute top-1/2 h-2.5 w-2.5 -translate-y-1/2 rounded-full pointer-events-none z-0",
      ...
      {/* Button Text & Icon (Z-10 guarantees text is crisp and never overlaps dot) */}
      <span className={cn(
        "relative z-10 flex items-center gap-2 font-ibm",
        isArabic ? "pr-3" : "pl-3"
      )}>
